...
function openInstitutionWindow() {
	institutionwindow = dhtmlmodal.open('Institution', 'iframe',
			'Institution.do?operation=add&pageOf=pageOfPopUpInstitution',
			'Administrative Data',
			'width=530px,height=175px,center=1,resize=0,scrolling=1')
...
function openDepartmentWindow() {
	departmentWindow = dhtmlmodal
			.open(
					'Department',
					'iframe',
					'Department.do?operation=add&pageOf=pageOfPopUpDepartment&menuSelected=3&submittedFor=AddNew',
					'Administrative Data',
					'width=530px,height=175px,center=1,resize=0,scrolling=1')
...
function openCRGWindow() {
	crgWindow = dhtmlmodal
			.open(
					'Cancer Research Group',
					'iframe',
					'CancerResearchGroup.do?operation=add&pageOf=pageOfPopUpCancerResearchGroup&menuSelected=4&submittedFor=AddNew',
					'Administrative Data',
					'width=530px,height=175px,center=1,resize=0,scrolling=1')
...
function sendRequestsWithData(url, data, cpOperation, pageOf) {
	...
	request.onreadystatechange = function requestHandler() {
		if (request.readyState == 4) { // When response is loaded
			if (request.status == 200) {
				...
				if (jsonResponse.locations != null) {
					...
					var eleOfActionSelBox = document
							.getElementById('actionIds');
					...
					for (i = 0; i < num; i++) {
						...
						if (cpOperation == "getUsersForThisSites") {
							...
							if ((selectedEditableUserId != null)
									&& (theValue == selectedEditableUserId)) {
							...
							if ((selectedEditableCPId != null)
									&& (theValue == selectedEditableCPId)) {
								document.getElementById('cpIds').options[i].selected = true;
							}
							if (i == (num - 1)) {
								getActionsForThisCPs('cpIds', 'siteIds',
										'roleIds', 'cpCheckId');
								break;
							}
						}
						...
						// Privileges List for Selected Roles
							...
							selectedOptsList(eleOfActionSelBox, optsList,
									selOptsList);
							...
							var optsList = jsonResponse.locations[i].actionJsonArray;
							var selOptsList = jsonResponse.locations[i].selectedActionArray;
							var actionSelBox = document
									.getElementById('actionIds');

							selectedOptsList(actionSelBox, optsList,
									selOptsList);
						}
...
function addOrUpdateRowToTable(opt, tableId, rowId, roleName, tempName, sites,
		actions, pageOf) {
	...
	sname = "<input type='checkbox' name='" + chkName + "' id='" + chkName
			+ "'/>";
	aprCheckb.onclick = function() {
		enableDeleteButton('summaryTableId', 'deleteButtonId');
	};
	...
	aprSites.width = "27%";
	// aprSites.innerHTML="<div
	// style='word-wrap:break-word;width=24%;'>"+sites+"</div>";
	aprSites.innerHTML = "<span>" + newSites + "</span>";
	aprSites.onmouseover = function() {
		Tip(sites, WIDTH, 200);
	};
	...
	aprActions.onmouseover = function() {
		Tip(actions, WIDTH, 200);
	};
 ...
 * var message=""; if(errorFlagForSite) { message = "<li> <font
 * color='red'>Site is required.</font> </li>"; } if(errorFlagForUser) {
 * message = message+"<li> <font color='red'>User is required.</font> </li>"; }
 * 
 * if(errorFlagForRole) { message = message+"<li> <font color='red'>Role is
 * required.</font> </li>"; } if(errorFlagForAction) { message = message+"<li>
 * <font color='red'>Privileges is required.</font> </li>"; }
 * 
 * if(message!=null) { divId.style.display = 'block'; divId.innerHTML=message; } }
 */
...
function switchToTab(selectedTab) {
	...
	var display6 = document.getElementById('defineDashboardItemsTable');

	display6.style.display = displayTable;

	if (selectedTab == "defineDashboardItemsTab") {

		display6.style.display = displayTable;
		display4.style.display = "none";

	} else if (selectedTab == "consentTab") {
		display4.style.display = displayTable;
		display6.style.display = "none";

	}

	var cpTab = document.getElementById('collectionProtocolTab');
	var consentTab = document.getElementById('consentTab');
	var defineDashboardItemsTab = document.getElementById('defineDashboardItemsTab');

	if (selectedTab == "collectionProtocolTab") {
		updateTab(cpTab, consentTab, defineDashboardItemsTab);
	} else if (selectedTab == "defineDashboardItemsTab") {
		updateTab(defineDashboardItemsTab, consentTab, cpTab);
	} else {
		updateTab(consentTab, cpTab, defineDashboardItemsTab);
	}
}
// This function is for changing the behaviour of TABs
function updateTab(tab1, tab2, tab3) {
	...
	tab2.onmouseover = function() {
		changeMenuStyle(this, ''), showCursor();
	};
	tab2.onmouseout = function() {
		changeMenuStyle(this, ''), hideCursor();
	};
...

{
	...
	var action = "DefineEvents.do?pageOf=pageOfAssignPrivilegePage&cpOperation=AssignPrivilegePage&operation="
			+ operation;
...
function defineDashboardItems() {

	switchToTab("defineDashboardItemsTab");
}
...
function preEventsOnRoleSelect(roleObject, cpCheckId) {
	...
	if (selectedRoleIds == "13") {
		...
		document.getElementById(cpCheckId).checked = true;
		document.getElementById(cpCheckId).onchange();
...
function editRowForCPPage(jsonResponse, i) {
	...
	var message = "<span class='black_ar'><strong>&nbsp;Edit Privilege For "
			+ selectedUserName + "</strong> </span></td></tr>";
	;
...
function getActionsForThisCPs(cpSelBoxId, siteSelBoxId, roleSelBoxId,
	...
	if (selectedCPIds != null && (selectedCPIds.length > 0)) {
		...
		var url = "ShowAssignPrivilegePage.do";
		var data = "cpOperation=" + cpOperation + "&selectedCPIds="
				+ selectedCPIds + "&selectedRoleIds=" + selectedRoleIds;
		sendRequestsWithData(url, data, cpOperation);
	} else if (selectedSiteIds != null && (selectedSiteIds.length > 0)) {
		cpOperation = "getActionsForThisSites";

		var url = "ShowAssignPrivilegePage.do";
		var data = "cpOperation=" + cpOperation + "&selectedSiteIds="
				+ selectedSiteIds + "&selectedRoleIds=" + selectedRoleIds
				+ "&isAllCPChecked=" + isAllCPChecked;
		sendRequestsWithData(url, data, cpOperation);
	} else {
		...
		var url = "ShowAssignPrivilegePage.do?pageOf=" + pageOf;
		var data = "cpOperation=" + cpOperation + "&selectedRoleIds="
				+ selectedRoleIds + "&selectedSiteIds=" + selectedSiteIds
				+ "&selectedCPIds=" + selectedCPIds + "&isAllCPChecked="
				+ isAllCPChecked;
		sendRequestsWithData(url, data, cpOperation);
	}
}
...
function getUserPrivilegeSummary(operation) {
	...
	if (pageOf != null && (pageOf == "pageOfAssignPrivilegePage"|| pageOf == "pageOfCollectionProtocol")) {
		...
		data = "cpOperation=" + cpOperation + "&selectedSiteIds="
				+ selectedSiteIds + "&selectedUserIds=" + selectedUserIds
				+ "&selectedRoleIds=" + selectedRoleIds + "&selectedActionIds="
				+ selectedActionIds + "&isCustChecked=" + isCustChecked
				+ "&operation=" + operation;
	} else if (pageOf != null) {
		...
		var data = "cpOperation=" + cpOperation + "&selectedSiteIds="
				+ selectedSiteIds + "&selectedCPIds=" + selectedCPIds
				+ "&selectedRoleIds=" + selectedRoleIds + "&selectedActionIds="
				+ selectedActionIds + "&isAllCPChecked=" + isAllCPChecked
				+ "&operation=" + operation;
	}

	isErrors = validateMethodForPriv(pageOf, selectedSiteIds, selectedRoleIds,
			selectedActionIds, selectedUserIds, selectedCPIds, isAllCPChecked,
			isCustChecked);
...
function validateMethodForPriv(pageOf, selectedSiteIds, selectedRoleIds,
		selectedActionIds, selectedUserIds, selectedCPIds, isAllCPChecked,
		isCustChecked) {
	var isErrors = false;
	...
	if (pageOf != null && (pageOf == "pageOfAssignPrivilegePage" || pageOf == "pageOfCollectionProtocol")) {
		...
		if (errorFlagForSite == true || errorFlagForUser == true
				|| errorFlagForAction == true || errorFlagForRole == true) {
			isErrors = true;

			displayErrorMessForPriv(pageOf, divId, errorFlagForSite,
					errorFlagForRole, errorFlagForAction, errorFlagForCP,
					errorFlagForUser);
		}
	} else if (pageOf != null) {
		...
		if (errorFlagForSite == true || errorFlagForAction == true
				|| errorFlagForRole == true || errorFlagForCP == true) {
			isErrors = true;
			displayErrorMessForPriv(pageOf, divId, errorFlagForSite,
					errorFlagForRole, errorFlagForAction, errorFlagForCP,
					errorFlagForUser);
...
function displayErrorMessForPriv(pageOf, divId, errorFlagForSite,
		errorFlagForRole, errorFlagForAction, errorFlagForCP, errorFlagForUser) {
	var message = "";
	...
	if (message != null) {
		...
		divId.innerHTML = "<tr><td width='16'>&nbsp;</td><td>" + message
				+ "</td></tr>";
	}
}
...
// To show element selected of a given select Box based on given slected input
// Id.
...
// disabling and enabling of selectBox on checkbox click and get Actions for
// Sites and role based on
// condition whether they are selected or not.

function eventOnChkBoxClick(checkBoxId, cpSelBoxId, siteSelBoxId, roleSelBoxId) {
	var cpId = document.getElementById("cpIds");
	if (document.getElementById("cpCheckId").checked == false) {
		cpId.className = "formFieldSizedNew";
	} else {
		cpId.className = "formFielDisabled";
	}
...
function onChkBoxClickForCP(customizeChkId, userSelBoxId, siteSelBoxId,
		roleSelBoxId, actionSelBoxId) {
	var checkBoxObj = document.getElementById(customizeChkId);
	var roleSelBoxObj = document.getElementById(roleSelBoxId);
