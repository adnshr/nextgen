...
public class ShipmentRequestBizLogic extends BaseShipmentBizLogic
{
	 ...
	 *  logger object
	 */
	private static transient Logger logger = Logger.getCommonLogger( ShipmentRequestBizLogic.class );
	 ...
	 */
	@Override
	public boolean validate(final Object obj, final DAO dao, final String operation)
			throws BizLogicException
	{
		boolean isValid = true;
		boolean isSpecimenVirtual = false;
		...
		if (obj instanceof ShipmentRequest == false)
		{
			...
			throw new BizLogicException( ErrorKey.getErrorKey( "errors.invalid.object.passed" ) ,
					null , "object is not the instance of ShipMent request class" );
		}
		...
		*/
		try
		{
							...
							.getActivityStatus().equals( Constants.ACTIVITY_STATUS_REJECTED ) ))
			{
						...
						if (container != null)
						{
									...
									&& !container.getBarcode().trim().equals( "" ))
							{
								...
								container = this.getContainerByNameOrBarcode( container
										.getBarcode(), dao, Constants.CONTAINER_PROPERTY_BARCODE );
							}
							if (container == null)
							{
				...
				{
					isEmptyContColl = true;
				}
				boolean isEmptySpecColl = false;
						...
						if (specimen != null)
						{
							...
							else
							{
								isValid = this.specimenBelongsToSite( specimen, shipmentRequest
										.getSenderSite().getId() );
								isInTransit = this.specimenBelongsToInTransitSite( specimen );
								isSpecimenVirtual = this.specimenBelongsToVirtualSite( specimen );
								if (isValid && !isInTransit)
								{
									throw this.getBizLogicException( null,
				...
				{
					isEmptySpecColl = true;
				}
				if (isEmptyContColl && isEmptySpecColl)
				{
	 ...
	 */
	private boolean specimenBelongsToInTransitSite(final Specimen specimen)
	{
		...
		if (specimen != null
				...
				&& specimen.getSpecimenPosition().getStorageContainer().getName().contains(
						Constants.IN_TRANSIT_CONTAINER_NAME_PREFIX ))
		{
			belongsTo = true;
		}
	 ...
	 * @param specimen to be checked.
	 * @param siteID of the site to be checked.
	 * @return true/false the result of the operation.
	 */
	private boolean specimenBelongsToSite(final Specimen specimen, final Long siteID)
	{
		boolean belongsTo = false;
		if (specimen != null
				&& specimen.getSpecimenPosition() != null
				&& specimen.getSpecimenPosition().getStorageContainer() != null
				&& specimen.getSpecimenPosition().getStorageContainer().getName() != null
				&& specimen.getSpecimenPosition().getStorageContainer().getSite().getId().equals(
						siteID ))
		{
			belongsTo = true;
		}
	 ...
	 */
	private boolean specimenBelongsToVirtualSite(final Specimen specimen)
	{
			...
			throws BizLogicException
	{
		try
		{
			...
			final Collection < StorageContainer > containerColl = this
			...
			shipmentRequest.getContainerCollection().addAll( containerColl );
	 ...
	 */
	@Override
	protected void setShipmentSystemProperties(final BaseShipment shipment)
	{
	...
	protected String getNotificationMailSubject(BaseShipment baseShipment)
	{
		...
		{
			mailSubject = ShipmentMailFormatterUtility
	...
	protected String getNotificationMailBody(BaseShipment baseShipment)
	{
		...
		if (baseShipment.getActivityStatus().equals( "Rejected" ))
		{
			mailBody = ShipmentMailFormatterUtility
	...
	public String getObjectId(DAO dao, Object domainObject)
	{
		...
		final StringBuffer tempStringBuff = new StringBuffer();
				...
				&& shipmentRequest.getActivityStatus().equals( Constants.ACTIVITY_STATUS_REJECTED ))
		{
			...
			{
				tempStringBuff.append( Site.class.getName() ).append( "_" ).append(
		...
		tempString = tempStringBuff.toString();
		return tempString;
	}
